...
export default (app, routes) => {
  ...
  if (process.env.AUTH_SOCIAL_LINKEDIN_CLIENT_ID) {
    passport.use(
      new LinkedInStrategy(
        {
          clientID:
            process.env.AUTH_SOCIAL_LINKEDIN_CLIENT_ID,
          clientSecret:
            process.env.AUTH_SOCIAL_LINKEDIN_CLIENT_SECRET,
          callbackURL:
            process.env.AUTH_SOCIAL_LINKEDIN_CALLBACK_URL,
...
function handleCallback(res, err, jwtToken) {
  if (err) {
    ...
    res.redirect(
      `${process.env.FRONTEND_URL}/signin?socialErrorCode=${errorCode}`,
    );
    return;
  }
