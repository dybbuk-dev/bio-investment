...
export default (app) => {
  ...
  app.post(
    ...
    require('./authSendEmailToSupport').default,
  );

  app.post(
    `/auth/send-email-on-comingSoon`,
    require('./authSendEmailOnComingSoon').default,
  );
  ...
  app.post(
    ...
    require('./authInvitationAccept').default,
  );
  app.delete(
    `/auth/invitation/:token/decline`,
    require('./authInvitationDecline').default,
  );
};
