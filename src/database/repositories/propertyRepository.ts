...
class ProductRepository {
  static async create(data, options: IRepositoryOptions) {
    ...
    let rows =
      await MongooseRepository.wrapWithSessionIfExists(
        Property(options.database).find({}),
        options,
      );
    ...
    if (rows.length !== 0) {
      propertyNumber =
        rows[rows.length - 1].propertyNumber + 1;
    }
  ...
  static async update(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        Property(options.database).findOne({
          _id: id,
        }),
        options,
      );
    ...
    await Property(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await Property(options.database).deleteOne(
      { _id: id },
      options,
    );
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.propertyNumber) {
        criteriaAnd.push({
          ['propertyNumber']: {
            $eq: filter.propertyNumber,
          },
        });
      }
    ...
    const count =
      await MongooseRepository.wrapWithSessionIfExists(
        Property(options.database).countDocuments(criteria),
        options,
      );
